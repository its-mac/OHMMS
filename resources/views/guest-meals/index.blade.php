...
@section('content')
        ...
        <div class="card-body">
            <form method="GET" class="row">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <a href="{{ route('manager.guest-meals.index') }}" class="btn btn-light">Reset</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            ...
            <a href="{{ route('manager.guest-meals.reports') }}" class="btn btn-secondary btn-sm">Reports</a>
        </div>
                ...
                <tbody>
                    @forelse($guestMeals as $guestMeal)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('manager.guest-meals.show', $guestMeal) }}"
                                    class="btn btn-info btn-sm">View</a>

                                @if ($guestMeal->status === 'pending')
                                    <form method="POST" action="{{ route('manager.guest-meals.approve', $guestMeal) }}"
                                        class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    <form method="POST" action="{{ route('manager.guest-meals.reject', $guestMeal) }}"
                                        class="d-inline">
                                        @csrf
                                        @method('PATCH')
